...
class AirCargoProblem(Problem):
    ...
    def actions(self, state: str) -> list:
        ...
        #using propotional logic kb
        kb = PropKB()
        kb.tell(decode_state(state, self.state_map).pos_sentence())
        
        #idea: looping through all executalbe actions. An action  
        # is executable if it satisfies a problem's preconditions
        for action in self.actions_list:
            #assume action is possible
            is_possible = True

            #an action must be in the positive preconditions
            for clause in action.precond_pos:
                if clause not in kb.clauses:
                    is_possible = False
                    break
            #an action must not be in the negative preconditions
    ...
    def h_ignore_preconditions(self, node: Node):
        ...
        count = 0
        for clause in self.goal: #refer to goal_test() code
            if clause not in kb.clauses:
                count +=1
        return count
...
def air_cargo_p3() -> AirCargoProblem:
    ...
    neg = [expr('At(C2, SFO)'),
           ...
           expr('At(C2, ATL)'),
           expr('At(P2, ATL)'),
           expr('At(C4, JFK)'),  #c4-->c2
           expr('In(C4, P2)'),
           expr('At(C2, ORD)'),
           expr('At(P2, ORD)'),
           expr('At(C4, ATL)'), #c4->c3
           expr('At(C3, ORD)'),
           ]
    init = FluentState(pos, neg)
    goal = [expr('At(C1, JFK)'),
