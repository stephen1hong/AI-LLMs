...
class PlanningGraph():
        ...
        for action in self.all_actions:
            ...
            if a_node.prenodes.issubset(prev_s_level):
                for s_node in prev_s_level: 
                    # Connect between A nodfe and S node
    ...
    def add_literal_level(self, level):
        ...
        #iterate through all previous a_nodes level
        for a_node in prev_a_level:
            for s_node in a_node.effnodes:
                #connect between Node A and Node S
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        # TODO test for Inconsistent Effects between nodes
         # Ensure sure node_a1's pos_effects are not conflicted by node_a2's neg_effects
        for effect in node_a1.action.effect_add:
            if effect in node_a2.action.effect_rem:
                return True

        # Ensure node_a2's pos_effects are not conflicted by node_a1's neg_effects
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        # Check if no conflict between node_a1's effects and node_a2's neg_preconditions, 
        for effect in node_a1.action.effect_add:
            if effect in node_a2.action.precond_neg:
                return True
        # Check if no conflict between node_a2's effects and node_a1's neg_preconditions, 
        for effect in node_a2.action.effect_add:
            if effect in node_a1.action.precond_neg:
                return True

        # Check if no conflict between node_a1's neg effects and node_a2's pos_preconditions, 
        for effect in node_a1.action.effect_rem:
            if effect in node_a2.action.precond_pos:
                return True
        # Check if no conflict between node_a2's neg effects and node_a1's pos_preconditions, 
    ...
    def competing_needs_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        for node_a1 in node_a1.parents:
            for node_a2 in node_a2.parents:
    ...
    def inconsistent_support_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s):
        ...
        for node_s1 in node_s1.parents:
            for node_s2 in node_s2.parents:
                if not node_s1.is_mutex(node_s2):
                    return False
        return True
